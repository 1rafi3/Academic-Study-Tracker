...
import { backupApi, semesterApi } from '../api/academicApi.js';
import type {
  ISemester,
  IBackupPayload,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Card 2: Restore from Backup (Import) */}
        <div className="p-6 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-lg flex flex-col justify-between space-y-4">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
            ...
            <h3 className="text-base font-bold text-slate-100">Import & Restore Data</h3>
            <p className="text-xs text-slate-400 leading-relaxed">
              Restore your academic data from an exported JSON file. You will see a verification preview before any changes are applied, and existing data will never be duplicated.
            </p>
          </div>
          ...
          <div>
            ...
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 active:bg-slate-600 text-slate-200 text-xs font-semibold border border-slate-700 shadow-xs transition cursor-pointer"
            >
        ...
        <div className="p-6 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-lg flex flex-col justify-between space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <button
              type="button"
              onClick={() => handleDownloadCsv(backupApi.getAttendanceCsvUrl(activeSemester?._id))}
            ...
            <button
              type="button"
              onClick={() => handleDownloadCsv(backupApi.getCoursesCsvUrl(activeSemester?._id))}
              className="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-200 text-xs font-semibold transition cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={() => handleDownloadCsv(backupApi.getEventsCsvUrl(activeSemester?._id))}
              className="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-200 text-xs font-semibold transition cursor-pointer"
            >
